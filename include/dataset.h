...
private:
	...
	//! Random distribution on spherical boundary 
	Bodies boundary(int numBodies, int seed, int numSplit) {
		Bodies bodies(numBodies);                                 // Initialize bodies
		for (int i = 0; i < numSplit; i++, seed++) {              // Loop over partitions (if there are any)
			int begin = 0;                                          //  Begin index of bodies
			int end = bodies.size();                                //  End index of bodies
			...
			for (B_iter B = bodies.begin() + begin; B != bodies.begin() + end; B++) { // Loop over bodies
				...
				double r = 0.8 + 0.2 * drand48(); 
				B->X[0] = std::sin(theta) * std::cos(phi) * r;
				B->X[1] = std::sin(theta) * std::sin(phi) * r;				
				B->X[2] = std::cos(theta) * r;				
			}                                                       //  End loop over bodies
		}                                                         // End loop over partitions
		return bodies;
	}
...
public:
	...
	Bodies initBodies(int numBodies, const char * distribution,
		...
		case 'b':                                                 // Case boundary
			bodies = boundary(numBodies, mpirank, numSplit);         //  Boundary distribution in a r = 0.8-1 sphere
			break;                                                  // End case for boundary			
